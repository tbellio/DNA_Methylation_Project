...
## Controlling for other variables in each of these analyses
## Adding export to excel files 7-23-2022
...
cortex_se <- tximeta(cortex_coldata)
...
cortex_gene_se <- summarizeToGene(cortex_se.exons)
...
library('DESeq2')
dds_APP_cortex_gene_se <- DESeqDataSet(APP_cortex_gene_se, design = ~ Sex + Age + Diet)
dds_APP_cortex_three_gene_se <- DESeqDataSet(APP_cortex_three_gene_se, design = ~ Sex + Diet)
dds_APP_cortex_six_gene_se <- DESeqDataSet(APP_cortex_six_gene_se, design = ~ Sex + Diet)
dds_APP_cortex_nine_gene_se <- DESeqDataSet(APP_cortex_nine_gene_se, design = ~ Sex + Diet)
dds_APP_cortex_twelve_gene_se <- DESeqDataSet(APP_cortex_twelve_gene_se, design = ~ Sex + Diet)

dds_WT_cortex_gene_se <- DESeqDataSet(WT_cortex_gene_se, design = ~ Sex + Age + Diet)
dds_WT_cortex_three_gene_se <- DESeqDataSet(WT_cortex_three_gene_se, design = ~ Sex + Diet)
dds_WT_cortex_six_gene_se <- DESeqDataSet(WT_cortex_six_gene_se, design = ~ Sex + Diet)
dds_WT_cortex_nine_gene_se <- DESeqDataSet(WT_cortex_nine_gene_se, design = ~ Sex + Diet)
dds_WT_cortex_twelve_gene_se <- DESeqDataSet(WT_cortex_twelve_gene_se, design = ~ Sex + Diet)

dds_cortex_gene_se <- DESeqDataSet(cortex_gene_se, design= ~ Age + Sex + APP + Diet)
...
dds_cortex_gene_se_10filtered$Diet <- relevel(dds_cortex_gene_se_10filtered$Diet, ref = "Control")
...
gene_expression_pval_ordered_from_dge_dds_cortex_gene_se_10filtered <- gene_expression_from_dge_dds_cortex_gene_se_10filtered[order(gene_expression_from_dge_dds_cortex_gene_se_10filtered$pvalue),]


## All Combined Effects of Diet
summary(results(dge_dds_cortex_gene_se_10filtered, alpha = 0.05))
## 3 genes were upregulated and 45 were downregulated in supplemented mice
summary(results(dge_dds_cortex_gene_se_10filtered))
## 5 genes were upregulated and 48 were downregulated in supplemented mice


## Diet Effects Separated by APP Genotype
...
summary(results(dge_dds_WT_cortex_gene_se_10filtered))
## no genes differentially expressed in supplemented WT mice
summary(results(dge_dds_APP_cortex_gene_se_10filtered))
## 1 gene was downregulated in supplemented APP mice
...
## Output is that of the 18837 genes,9 genes were upregulated and 14 were downregulated in supplemented 3 motnh old APP mice
summary(results(dge_dds_WT_cortex_three_gene_se_10filtered, alpha = 0.05))
## Output is that of the 18837 genes,1 genes were upregulated and 0 were downregulated in supplemented 3 motnh old WT mice
## Get a summary of DEGs with FDR (p adj) of <0.1
summary(results(dge_dds_APP_cortex_three_gene_se_10filtered))
## Output is that of the 18837 genes,15 genes were upregulated and 28 were downregulated in supplemented 3 motnh old APP mice
summary(results(dge_dds_WT_cortex_three_gene_se_10filtered))
## Output is that of the 18837 genes,1 genes were upregulated and 0 were downregulated in supplemented 3 motnh old WT mice
...
##Output is that of the 19242 genes, 1 gene was upregulated and 3 genes were downregulated in supp mice 6mo APP mice
summary(results(dge_dds_WT_cortex_six_gene_se_10filtered, alpha = 0.05))
## Output is that of the 19242 genes,2 genes were upregulated and 1 were downregulated in supplemented 6 motnh old WT mice
## Get a summary of DEGs with FDR (p adj) of <0.1
summary(results(dge_dds_APP_cortex_six_gene_se_10filtered))
##Output is that of the 19242 genes, 1 gene was upregulated and 3 genes were downregulated in supp mice 6mo APP mice
summary(results(dge_dds_WT_cortex_six_gene_se_10filtered))
## Output is that of the 19242 genes,4 genes were upregulated and 1 were downregulated in supplemented 6 motnh old WT mice
...
## Get a summary of DEGs with FDR (p adj) of <0.1
summary(results(dge_dds_APP_cortex_nine_gene_se_10filtered))
##Output is that of the 19225 genes, no genes were differentially expressed in supplemented 9mo APP mice
summary(results(dge_dds_WT_cortex_nine_gene_se_10filtered))
##Output is that of the 19225 genes, no genes were differentially expressed in supplemented 9mo WT mice
...
##Output is that of the 18741 genes, 0 genes was upregulated and 3 gene was downregulated in supp APP mice at 12 months
...
## Get a summary of DEGs with FDR (p adj) of <0.1
summary(results(dge_dds_APP_cortex_twelve_gene_se_10filtered))
##Output is that of the 18741 genes, 0 genes was upregulated and 3 gene was downregulated in supp APP mice at 12 months
summary(results(dge_dds_WT_cortex_twelve_gene_se_10filtered))
##Output is that of the 18741 genes, 2 genes were upregulated in supp WT mice at 12 months
...
write.csv(as.data.frame(gene_expression_padj_ordered_from_dge_dds_APP_cortex_gene_se_10filtered), file = 'Batch_of_192_Trimmed_Cortex_DESeq2_Control_v_Supp_APP_Only_DGE_Analysis_sex_age_controlled_padj_sorted_7_23_2022.csv')
write.csv(as.data.frame(gene_expression_padj_ordered_from_dge_dds_APP_cortex_three_gene_se_10filtered), file = 'Batch_of_192_Trimmed_Cortex_DESeq2_Control_v_Supp_3mo_APP_DGE_Analysis_sex_controlled_padj_sorted_7_23_2022.csv')
write.csv(as.data.frame(gene_expression_padj_ordered_from_dge_dds_APP_cortex_six_gene_se_10filtered), file = "Batch_of_192_Trimmed_Cortex_DESeq2_Control_v_Supp_6mo_APP_DGE_Analysis_sex_controlled_padj_sorted_7_23_2022.csv")
write.csv(as.data.frame(gene_expression_padj_ordered_from_dge_dds_APP_cortex_nine_gene_se_10filtered), file = "Batch_of_192_Trimmed_Cortex_DESeq2_Control_v_Supp_9mo_APP_DGE_Analysis_sex_controlled_padj_sorted_7_23_2022.csv")
write.csv(as.data.frame(gene_expression_padj_ordered_from_dge_dds_APP_cortex_twelve_gene_se_10filtered), file = "Batch_of_192_Trimmed_Cortex_DESeq2_Control_v_Supp_12mo_APP_DGE_Analysis_sex_controlled_padj_sorted_7_23_2022.csv")

write.csv(as.data.frame(gene_expression_padj_ordered_from_dge_dds_WT_cortex_gene_se_10filtered), file = 'Batch_of_192_Trimmed_Cortex_DESeq2_Control_v_Supp_WT_only_DGE_Analysis_age_sex_controlled_padj_sorted_7_23_2022.csv')
write.csv(as.data.frame(gene_expression_padj_ordered_from_dge_dds_WT_cortex_three_gene_se_10filtered), file = 'Batch_of_192_Trimmed_Cortex_DESeq2_Control_v_Supp_3mo_WT_DGE_Analysis_sex_controlled_padj_sorted_7_23_2022.csv')
write.csv(as.data.frame(gene_expression_padj_ordered_from_dge_dds_WT_cortex_six_gene_se_10filtered), file = "Batch_of_192_Trimmed_Cortex_DESeq2_Control_v_Supp_6mo_WT_DGE_Analysis_sex_controlled_padj_sorted_7_23_2022.csv")
write.csv(as.data.frame(gene_expression_padj_ordered_from_dge_dds_WT_cortex_nine_gene_se_10filtered), file = "Batch_of_192_Trimmed_Cortex_DESeq2_Control_v_Supp_9mo_WT_DGE_Analysis_sex_controlled_padj_sorted_7_23_2022.csv")
write.csv(as.data.frame(gene_expression_padj_ordered_from_dge_dds_WT_cortex_twelve_gene_se_10filtered), file = "Batch_of_192_Trimmed_Cortex_DESeq2_Control_v_Supp_12mo_WT_DGE_Analysis_sex_controlled_padj_sorted_7_23_2022.csv")

write.csv(as.data.frame(gene_expression_padj_ordered_from_dge_dds_cortex_gene_se_10filtered), file = 'Batch_of_192_Trimmed_Cortex_DESeq2_Control_v_Supp_All_Animals_DGE_Analysis_app_age_sex_controlled_padj_sorted_7_23_2022.csv')

write.csv(as.data.frame(gene_expression_pval_ordered_from_dge_dds_APP_cortex_gene_se_10filtered), file = 'Batch_of_192_Trimmed_Cortex_DESeq2_Control_v_Supp_only_APP_DGE_Analysis_sex_age_controlled_pval_sorted_7_23_2022.csv')
write.csv(as.data.frame(gene_expression_pval_ordered_from_dge_dds_APP_cortex_three_gene_se_10filtered), file = 'Batch_of_192_Trimmed_Cortex_DESeq2_Control_v_Supp_3mo_APP_DGE_Analysis_sex_controlled_pval_sorted_7_23_2022.csv')
write.csv(as.data.frame(gene_expression_pval_ordered_from_dge_dds_APP_cortex_six_gene_se_10filtered), file = "Batch_of_192_Trimmed_Cortex_DESeq2_Control_v_Supp_6mo_APP_DGE_Analysis_sex_controlled_pval_sorted_7_23_2022.csv")
write.csv(as.data.frame(gene_expression_pval_ordered_from_dge_dds_APP_cortex_nine_gene_se_10filtered), file = "Batch_of_192_Trimmed_Cortex_DESeq2_Control_v_Supp_9mo_APP_DGE_Analysis_sex_controlled_pval_sorted_7_23_2022.csv")
write.csv(as.data.frame(gene_expression_pval_ordered_from_dge_dds_APP_cortex_twelve_gene_se_10filtered), file = "Batch_of_192_Trimmed_Cortex_DESeq2_Control_v_Supp_12mo_APP_DGE_Analysis_sex_controlled_pval_sorted_7_23_2022.csv")

write.csv(as.data.frame(gene_expression_pval_ordered_from_dge_dds_WT_cortex_gene_se_10filtered), file = 'Batch_of_192_Trimmed_Cortex_DESeq2_Control_v_Supp_WT_only_DGE_Analysis_age_sex_controlled_pval_sorted_7_23_2022.csv')
write.csv(as.data.frame(gene_expression_pval_ordered_from_dge_dds_WT_cortex_three_gene_se_10filtered), file = 'Batch_of_192_Trimmed_Cortex_DESeq2_Control_v_Supp_3mo_WT_DGE_Analysis_sex_controlled_pval_sorted_7_23_2022.csv')
write.csv(as.data.frame(gene_expression_pval_ordered_from_dge_dds_WT_cortex_six_gene_se_10filtered), file = "Batch_of_192_Trimmed_Cortex_DESeq2_Control_v_Supp_6mo_WT_DGE_Analysis_sex_controlled_pval_sorted_7_23_2022.csv")
write.csv(as.data.frame(gene_expression_pval_ordered_from_dge_dds_WT_cortex_nine_gene_se_10filtered), file = "Batch_of_192_Trimmed_Cortex_DESeq2_Control_v_Supp_9mo_WT_DGE_Analysis_sex_controlled_pval_sorted_7_23_2022.csv")
write.csv(as.data.frame(gene_expression_pval_ordered_from_dge_dds_WT_cortex_twelve_gene_se_10filtered), file = "Batch_of_192_Trimmed_Cortex_DESeq2_Control_v_Supp_12mo_WT_DGE_Analysis_sex_controlled_pval_sorted_7_23_2022.csv")

write.csv(as.data.frame(gene_expression_pval_ordered_from_dge_dds_cortex_gene_se_10filtered), file = 'Batch_of_192_Trimmed_Cortex_DESeq2_Control_v_Supp_All_animals_DGE_Analysis_app_age_sex_controlled_pval_sorted_7_23_2022.csv')
