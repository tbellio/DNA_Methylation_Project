...
library('DESeq2')
dds_cortex_three_gene_se <- DESeqDataSet(cortex_three_gene_se, design = ~ Diet)
dds_cortex_six_gene_se <- DESeqDataSet(cortex_six_gene_se, design = ~ Diet)
dds_cortex_nine_gene_se <- DESeqDataSet(cortex_nine_gene_se, design = ~ Diet)
dds_cortex_twelve_gene_se <- DESeqDataSet(cortex_twelve_gene_se, design = ~ Diet)
...
dds_cortex_three_gene_se_10filtered$Diet <- relevel(dds_cortex_three_gene_se_10filtered$Diet, ref = "Control")
dds_cortex_six_gene_se_10filtered$Diet <- relevel(dds_cortex_six_gene_se_10filtered$Diet, ref = "Control")
dds_cortex_nine_gene_se_10filtered$Diet <- relevel(dds_cortex_nine_gene_se_10filtered$Diet, ref = "Control")
dds_cortex_twelve_gene_se_10filtered$Diet <- relevel(dds_cortex_twelve_gene_se_10filtered$Diet, ref = "Control")
## Run DGE analysis
...
gene_expression_padj_ordered_from_dge_dds_cortex_six_gene_se_10filtered 

## Get a summary of DEGs with FDR (p adj) of <0.05
summary(results(dge_dds_cortex_three_gene_se_10filtered, alpha = 0.05))
## Output is that of the 18837 genes,0 genes were affected between control and supp diet
summary(results(dge_dds_cortex_six_gene_se_10filtered, alpha = 0.05))
##Output is that of the 19242 genes, 2 genes were downregulated in supp mice
summary(results(dge_dds_cortex_nine_gene_se_10filtered, alpha = 0.05))
##Output is that of the 19225 genes, 1 gene was upregulated in supp mice at 9 months
summary(results(dge_dds_cortex_twelve_gene_se_10filtered, alpha = 0.05))
##Output is that of the 18741 genes, 1 genes was upregulated supp mice at 12 months
...
write.csv(as.data.frame(gene_expression_padj_ordered_from_dge_dds_cortex_three_gene_se_10filtered), file = 'Batch_of_192_Trimmed_Cortex_DESeq2_Control_v_Supp_3mo_DGE_Analysis_padj_sorted_6_8_2022.csv')
write.csv(as.data.frame(gene_expression_padj_ordered_from_dge_dds_cortex_six_gene_se_10filtered), file = "Batch_of_192_Trimmed_Cortex_DESeq2_Control_v_Supp_6mo_DGE_Analysis_padj_sorted_6_8_2022.csv")
write.csv(as.data.frame(gene_expression_padj_ordered_from_dge_dds_cortex_nine_gene_se_10filtered), file = "Batch_of_192_Trimmed_Cortex_DESeq2_Control_v_Supp_9mo_DGE_Analysis_padj_sorted_6_8_2022.csv")
write.csv(as.data.frame(gene_expression_padj_ordered_from_dge_dds_cortex_twelve_gene_se_10filtered), file = "Batch_of_192_Trimmed_Cortex_DESeq2_Control_v_Supp_12mo_DGE_Analysis_padj_sorted_6_8_2022.csv")
