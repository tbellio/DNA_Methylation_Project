## Looking for the effects of RIN on DEGs
...
## Starting this on 8/17/2022
## Using DESeq2 to get DEGs for WT vs APP, trying to control for age, sex, diet, and RIN
...
cortex_coldata <- data.frame(files, names= sample_metadata$names, APP= sample_metadata$APP, Age= sample_metadata$Age, Diet= sample_metadata$Diet, Sex= sample_metadata$Sex,
                             RIN= sample_metadata$Cortex.RIN, stringsAsFactors = F)
cortex_coldata
...
library('DESeq2')
dds_cortex_gene_se <- DESeqDataSet(cortex_gene_se, design = ~ Age + Diet + Sex + RIN + APP) ## this will create a DESEqDataSet with the design saying look for the effects of APP while controlling for age, diet, sex
...
## 3049 genes are upregulated and 2815 genes are downegulated in APP mice compared to WT with padj < 0.05; this is controlling for age, sex, diet, RIN
summary(APP_cortex_results)
## 3328 genes are upregulated and 3627 genes are downregulated in APP mice compared to WT with padj < 0.1; this is controlling for age, sex, diet, RIN
...
library('DESeq2')
dds_cortex_gene_se <- DESeqDataSet(cortex_gene_se, design = ~ Age + Diet + Sex + APP + RIN)
## Filter so that only genes that have more than 10 reads total are inculuded
dds_cortex_gene_se_10filtered <- dds_cortex_gene_se[rowSums(counts(dds_cortex_gene_se)) >= 10,]
...
results(dge_dds_cortex_gene_se_10filtered, contrast = c("RIN", "8.5", "2.7"))
summary(results(dge_dds_cortex_gene_se_10filtered, contrast = c("RIN", "8.5", "4.9")))
