...
hippo__three_se <- tximeta(hippo_three_only_coldata)  ##creates se which is a very large summarized experiment dataset....IT FUCKING WORKED...well maybe
hippo_six_se <- tximeta(hippo_six_only_coldata)
hippo_nine_se <- tximeta(hippo_nine_only_coldata)
hippo_twelve_se <- tximeta(hippo_twelve_only_coldata)
...
colData(hippo__three_se)
colData(hippo_six_se)
colData(hippo_nine_se)
colData(hippo_twelve_se)
## Check the assay names; should be 3- counts, abundance, length
assayNames(hippo__three_se)
assayNames(hippo_six_se)
assayNames(hippo_nine_se)
assayNames(hippo_twelve_se)
## Check the row ranges
rowRanges(hippo__three_se)
rowRanges(hippo_six_se)
rowRanges(hippo_nine_se)
rowRanges(hippo_twelve_se)
## Check the seqinfo
seqinfo(hippo__three_se)
seqinfo(hippo_six_se)
seqinfo(hippo_nine_se)
seqinfo(hippo_twelve_se)
## get the database that was used to match the Salmon input quants
edb <- retrieveDb(hippo__three_se)
class(edb)

##adding exons to the summarized experiment (hippo__three_se, etc.)
hippo_three_se.exons <- addExons(hippo__three_se)
hippo_six_se.exons <- addExons(hippo_six_se)
hippo_nine_se.exons <- addExons(hippo_nine_se)
hippo_twelve_se.exons <- addExons(hippo_twelve_se)

##Check rowranges--shouldnt have to make sure it works for all four as same code
rowRanges(hippo_three_se.exons)

## Summarize read counts to the gene level
hippo_three_gene_se <- summarizeToGene(hippo_three_se.exons)
hippo_six_gene_se <- summarizeToGene(hippo_six_se.exons)
hippo_nine_gene_se <- summarizeToGene(hippo_nine_se.exons)
hippo_twelve_gene_se <- summarizeToGene(hippo_twelve_se.exons)

rowRanges(hippo_three_gene_se)


## Now use DESeq2 to look for DEGs at each time point
library('DESeq2')
dds_hippo_three_gene_se <- DESeqDataSet(hippo_three_gene_se, design = ~ APP)
dds_hippo_six_gene_se <- DESeqDataSet(hippo_six_gene_se, design = ~ APP)
dds_hippo_nine_gene_se <- DESeqDataSet(hippo_nine_gene_se, design = ~ APP)
dds_hippo_twelve_gene_se <- DESeqDataSet(hippo_twelve_gene_se, design = ~ APP)
...
dds_hippo_three_gene_se_10filtered <- dds_hippo_three_gene_se[rowSums(counts(dds_hippo_three_gene_se)) >= 10,]
...
dds_hippo_six_gene_se_10filtered$APP <- relevel(dds_hippo_six_gene_se_10filtered$APP, ref = "WT")
dds_hippo_nine_gene_se_10filtered$APP <- relevel(dds_hippo_nine_gene_se_10filtered$APP, ref = "WT")
dds_hippo_twelve_gene_se_10filtered$APP <- relevel(dds_hippo_twelve_gene_se_10filtered$APP, ref = "WT")
## Run DGE analysis
dge_dds_hippo_three_gene_se_10filtered <- DESeq(dds_hippo_three_gene_se_10filtered)
dge_dds_hippo_six_gene_se_10filtered <- DESeq(dds_hippo_six_gene_se_10filtered)
dge_dds_hippo_nine_gene_se_10filtered <- DESeq(dds_hippo_nine_gene_se_10filtered)
dge_dds_hippo_twelve_gene_se_10filtered <- DESeq(dds_hippo_twelve_gene_se_10filtered)

## See which comparisons were run; should only be one as model was just APP vs WT
resultsNames(dge_dds_hippo_three_gene_se_10filtered)
resultsNames(dge_dds_hippo_six_gene_se_10filtered)
resultsNames(dge_dds_hippo_nine_gene_se_10filtered)
resultsNames(dge_dds_hippo_twelve_gene_se_10filtered)

## Get gene expression table as DESeqResults
gene_expression_from_dge_dds_hippo_three_gene_se_10filtered <- results(dge_dds_hippo_three_gene_se_10filtered)
gene_expression_from_dge_dds_hippo_six_gene_se_10filtered <- results(dge_dds_hippo_six_gene_se_10filtered)
gene_expression_from_dge_dds_hippo_nine_gene_se_10filtered <- results(dge_dds_hippo_nine_gene_se_10filtered)
gene_expression_from_dge_dds_hippo_twelve_gene_se_10filtered <- results(dge_dds_hippo_twelve_gene_se_10filtered)
...
gene_expression_from_dge_dds_hippo_three_gene_se_10filtered
gene_expression_from_dge_dds_hippo_six_gene_se_10filtered
gene_expression_from_dge_dds_hippo_nine_gene_se_10filtered
gene_expression_from_dge_dds_hippo_twelve_gene_se_10filtered

## Sort the gene expression data by FDR(adjusted p-value)
gene_expression_padj_ordered_from_dge_dds_hippo_three_gene_se_10filtered <- gene_expression_from_dge_dds_hippo_three_gene_se_10filtered[order(gene_expression_from_dge_dds_hippo_three_gene_se_10filtered$padj),]
gene_expression_padj_ordered_from_dge_dds_hippo_six_gene_se_10filtered <- gene_expression_from_dge_dds_hippo_six_gene_se_10filtered[order(gene_expression_from_dge_dds_hippo_six_gene_se_10filtered$padj),]
gene_expression_padj_ordered_from_dge_dds_hippo_nine_gene_se_10filtered <- gene_expression_from_dge_dds_hippo_nine_gene_se_10filtered[order(gene_expression_from_dge_dds_hippo_nine_gene_se_10filtered$padj),]
gene_expression_padj_ordered_from_dge_dds_hippo_twelve_gene_se_10filtered <- gene_expression_from_dge_dds_hippo_twelve_gene_se_10filtered[order(gene_expression_from_dge_dds_hippo_twelve_gene_se_10filtered$padj),]
