...
dds_WT_hippo_twelve_gene_se <- DESeqDataSet(WT_hippo_twelve_gene_se, design = ~ Diet)
...
## Output is that of the 18837 genes,1 genes were upregulated and 2 were downregulated in supplemented 3 motnh old APP mice
summary(results(dge_dds_WT_hippo_three_gene_se_10filtered, alpha = 0.05))
## Output is that of the 18837 genes,1 genes were upregulated and 1 was downregulated in supplemented 3 motnh old WT mice


summary(results(dge_dds_APP_hippo_six_gene_se_10filtered, alpha = 0.05))
##Output is that of the 19242 genes, 12 genes were upregulated and 10 genes were downregulated in supp mice 6mo APP mice
summary(results(dge_dds_WT_hippo_six_gene_se_10filtered, alpha = 0.05))
## Output is that of the 19242 genes,0 genes were upregulated and 0 were downregulated in supplemented 6 motnh old WT mice


summary(results(dge_dds_APP_hippo_nine_gene_se_10filtered, alpha = 0.05))
##Output is that of the 19225 genes, 3 genes were upregulated and 2 genes were downregulated in supplemented 9mo APP mice
...
##Output is that of the 18741 genes, 185 genes was upregulated and 164 gene was downregulated in supp APP mice at 12 months
summary(results(dge_dds_WT_hippo_twelve_gene_se_10filtered, alpha = 0.05))
##Output is that of the 18741 genes, 2 genes were upregulated and 2 were downregulated in supp WT mice at 12 months
...
write.csv(as.data.frame(gene_expression_pval_ordered_from_dge_dds_WT_hippo_twelve_gene_se_10filtered), file = "Batch_of_192_Trimmed_Hippo_DESeq2_Control_v_Supp_12mo_WT_DGE_Analysis_pval_sorted_6_10_2022.csv")